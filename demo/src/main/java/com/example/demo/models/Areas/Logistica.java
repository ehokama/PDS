...
public class Logistica extends Area implements HandlerEtapa, IAreaObserver {
    ...
    private HandlerEtapa siguienteHandler = Entrega.getInstancia() ;;
    ...
    public void update(OrdenDeCompra ordenDeCompra) {
        System.out.println("Logistica recibió una actualización del pedido #" + ordenDeCompra.getNumeroPedido());
    ...
    public void handle(OrdenDeCompra ordenDeCompra) {
        ...
        System.out.println("Logistica está procesando el pedido #" + ordenDeCompra.getNumeroPedido()); //1. la procesa el area actual
        ...
        if(siguienteHandler!=null){
            ordenDeCompra.setEstadoPedido( new EstadoPedido(Entrega.getInstancia()) );            //4.  crea un nuevo estado, se lo asigna al nextHandler
