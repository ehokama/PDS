...
public class Entrega extends Area implements HandlerEtapa, IAreaObserver {
    ...
    private HandlerEtapa siguienteHandler = Seguimiento.getInstancia() ;;
    ...
    public void handle(OrdenDeCompra ordenDeCompra) {
        ...
        System.out.println("Entrega está procesando el pedido #" + ordenDeCompra.getNumeroPedido()); //1. la procesa el area actual
        ...
        if(siguienteHandler!=null){
            ordenDeCompra.setEstadoPedido( new EstadoPedido(Seguimiento.getInstancia()) );            //4.  crea un nuevo estado, se lo asigna al nextHandler
