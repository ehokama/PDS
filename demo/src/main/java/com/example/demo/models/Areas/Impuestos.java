...
public class Impuestos extends Area implements HandlerEtapa, IAreaObserver {
    ...
    private HandlerEtapa siguienteHandler = Embarque.getInstancia() ;;
    ...
    public void handle(OrdenDeCompra ordenDeCompra) {
        ...
        System.out.println("Impuestos está procesando el pedido #" + ordenDeCompra.getNumeroPedido()); //1. la procesa el area actual
        ...
        if(siguienteHandler!=null){
            ordenDeCompra.setEstadoPedido( new EstadoPedido(Embarque.getInstancia()) );            //4.  crea un nuevo estado, se lo asigna al nextHandler
